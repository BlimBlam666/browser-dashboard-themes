...
function bindEvents() {
  ...
  elements.clearNote.addEventListener("click", async () => {
    ...
    updateNoteCount();
    showToast(elements.toast, "The parchment is clear.");
  });
...
async function renderFocus() {
  ...
  elements.focusState.textContent = snapshot.running ? "ward held" : snapshot.remaining === 0 ? "complete" : "ready";
  elements.focusToggle.textContent = snapshot.running ? "Pause ritual" : snapshot.remaining === 0 ? "Begin anew" : "Begin ritual";
  for (const button of elements.durationButtons) {
  ...
  if (wasRunning && !snapshot.running && snapshot.remaining === 0 && !completionHandled) {
    ...
    showToast(elements.toast, "The ritual is complete.", 4000);
...
init().catch((error) => {
  console.error(error);
  showToast(elements.toast, "The Familiar could not awaken. Reopen the panel to try again.", 5000);
});
