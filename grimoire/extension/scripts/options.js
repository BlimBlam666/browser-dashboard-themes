...
const elements = {
  ...
  returnGrimoire: byId("return-grimoire"),
...
function bindEvents() {
  ...
  elements.returnGrimoire.addEventListener("click", () => chrome.tabs.create({ url: "chrome://newtab/" }));
}
...
async function savePreferences() {
  ...
  showToast(elements.toast, "The binding has been updated.", 1500);
}
...
async function togglePermission(permission) {
  ...
  if (granted) {
    ...
    showToast(elements.toast, "The optional door has been sealed.");
  } else {
    const accepted = await chrome.permissions.request(request);
    showToast(elements.toast, accepted ? "The optional door has been opened." : "The door remains sealed.");
  }
  await refreshPermissionButtons();
}
...
async function exportBackup() {
  ...
  const backup = {
    format: "grimoire-dashboard-backup",
    version: 1,
  ...
  link.href = href;
  link.download = `grimoire-backup-${getDayKey()}.json`;
  link.click();
...
async function importBackup(event) {
  ...
  if (file.size > 1_000_000) {
    elements.recordsStatus.textContent = "That file is too large to be a Grimoire backup.";
    return;
  }
  try {
    const parsed = JSON.parse(await file.text());
    const candidate = parsed?.format === "grimoire-dashboard-backup" ? parsed.state : parsed;
    ...
    showToast(elements.toast, "The Grimoire has remembered.", 3000);
  } catch (error) {
    console.error(error);
    elements.recordsStatus.textContent = "This does not appear to be a valid Grimoire backup.";
  }
}

async function resetAllData() {
  const accepted = window.confirm("Reset all portals, quests, notes, focus settings, and appearance choices? This cannot be undone unless you exported a backup.");
  if (!accepted) return;
  ...
  state = fresh;
  populateForm();
  elements.recordsStatus.textContent = "The Grimoire has returned to its first page.";
  showToast(elements.toast, "The binding has been renewed.", 3000);
}

init().catch((error) => {
  console.error(error);
  showToast(elements.toast, "The Binding Chamber could not open correctly. Reload to try again.", 5000);
});
